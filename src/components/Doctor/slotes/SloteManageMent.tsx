...
export default function DoctorAppointmentScheduler() {
  ...
  const [timeSlots, setTimeSlots] = useState<
    Record<string, Array<{ time: string; type: string }>>
  >({});
  ...
  console.log(
    "check this appointment data getting or not",
  ...
  useEffect(() => {
    if (savedAppointmentData?.success) {
      const slotData = savedAppointmentData;

      if (slotData.dates && slotData.dates.length > 0) {
        setSavedDates(slotData.dates);
        setDatesWithSlots(slotData.dates);
        setSlotsCreated(slotData.slots_created || 0);
        setShowSavedInfo(true);
      }

      if (slotData.timeSlots) {
        const formattedTimeSlots: Record<
          string,
          Array<{ time: string; type: string }>
        > = {};

        slotData.timeSlots.forEach(
          (dateSlot: { date: string; slots: string[] }) => {
              ...
              while (i < formattedTimeSlots[dateSlot.date].length) {
                ...
                if (i < formattedTimeSlots[dateSlot.date].length - 1) {
                  const slotTime = new Date(
                    `2023-01-01T${formattedTimeSlots[dateSlot.date][i].time}`
                  );
        ...
        if (slotData.includeRestPeriods !== undefined)
          setIncludeRestPeriods(slotData.includeRestPeriods);
      }
    }
  ...
  const generateTimeSlots = (start, end, appointmentDuration) => {
    ...
    let slotCount = 0;
    while (currentTime < endTime && slotCount < 100) {
      // Safety limit
      const timeString = currentTime.toTimeString().slice(0, 5);
      const displayTime = formatTimeForDisplay(timeString);

      console.log(`Slot ${slotCount + 1}: ${timeString} -> ${displayTime}`);
      ...
      if (includeRestPeriods && currentTime < endTime) {
        ...
        const restDisplayTime = formatTimeForDisplay(restTimeString);

        console.log(`Rest period: ${restTimeString} -> ${restDisplayTime}`);

        slots.push({
          time: restTimeString,
          displayTime: restDisplayTime,
          type: "rest",
        });
  ...
  const handleRemoveIndividualSlot = (
    dateString: string,
    slotIndex: number
  ) => {
    ...
    if (updatedTimeSlots[dateString]) {
      updatedTimeSlots[dateString] = updatedTimeSlots[dateString].filter(
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div>
                  ...
                  <div className="font-medium">
                    {dateRange === "oneWeek" ? "One Week" : "Two Weeks"}
                  </div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="font-medium">
                    {includeRestPeriods ? "Enabled (15min)" : "Disabled"}
                  </div>
                </div>
              </div>
            </div>
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      if (isEditingMode) {
        console.log(
          "......isEditingMode isEditingMode isEditingMode isEditingMode isEditingMode........."
        );
        ...
        appointmentSettings = {
          ...
          remainingSlots:
            savedAppointmentData?.result?.slots
      ...
      const response = await dispatch(
        storeAppointmentSlots(appointmentSettings)
      );
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <h2 className="text-xl font-semibold mb-4">
          Step 1: Select Doctor and Date Range
        </h2>
          ...
          <div className="flex items-center p-3 bg-blue-50 rounded-lg border border-blue-200">
            ...
            <label htmlFor="oneWeek" className="ml-2 text-gray-700">
              ...
              <p className="text-sm text-gray-500 mt-1">
                Select dates from the next 7 days
              </p>
            </label>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center">
          ...
          >
            {showSavedInfo && savedDates.length > 0
              ? "Create New Schedule"
              : "Next"}
  ...
  const renderStep2 = () => {
    ...
    const datesToShow = isEditingMode
      ? allDates.filter(
          (date) =>
            savedDates.includes(date.dateString) || timeSlots[date.dateString]
        )
      : allDates;
    ...
    const checkForExistingSlots = () => {
      ...
      const conflictingDates = selectedDates.filter(
        (date) => savedDates.includes(date) || timeSlots[date]
      );

      if (conflictingDates.length > 0) {
        const conflictingDateStrings = conflictingDates
          .map((date) => formatTimeForDisplay(date))
          .join(", ");
    ...
    return (
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        {isEditingMode && (
          <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="text-blue-700 text-sm">
              You are editing your existing schedule. Only your currently
              scheduled days are shown below.
            </p>
          </div>
        )}
        ...
        {!isEditingMode && (
          <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="text-blue-700 text-sm">
              Dates with existing slots are marked with a{" "}
              <span className="font-semibold">green indicator</span>. You can
              select them to edit existing slots or choose new dates to create
              slots.
            </p>
          </div>
        )}
        ...
        {isEditingMode && datesToShow.length === 0 && (
          <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
            <p className="text-yellow-700 text-sm">
              No scheduled dates found to edit. Please create a new schedule
              first.
            </p>
          </div>
        )}
  ...
  const renderStep3 = () => {
    ...
    const getExistingTimesForDate = (dateString) => {
      ...
      if (savedAppointmentData?.slots) {
        const existingSlotsForDate = savedAppointmentData.slots
          .filter(
            (slot) =>
              slot.date === dateString && !removedSlots.includes(slot.id)
          )
          .map((slot) => slot.time);
    ...
    const handleTimeSlotSetupWithAvoidance = (dateString) => {
      ...
      if (slots.length === 0) {
        alert(
          "No available time slots can be created. All times in the specified range already exist or conflict with existing appointments."
        );
        return;
      }
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <h2 className="text-xl font-semibold mb-4">
            {isEditingMode
              ? "Step 3: Edit Your Existing Time Slots"
          ...
          {isEditingMode ? (
            <div className="mb-6">
              <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-blue-700 text-sm">
                  <strong>Editing Mode:</strong> Below are your existing
                  appointment slots for the selected dates from Step 2. You can
                  remove individual slots or entire days as needed. You can also
                  add new slots to these dates.
                  <br />
                  <strong>Note:</strong> When adding new slots, the system will
                  automatically avoid conflicting with existing appointment
                  times.
                  <br />
                  <strong>Warning:</strong> Removing booked slots will cancel
                  user appointments.
                </p>
              </div>
                ...
                {selectedDates.length > 0 ? (
                  (() => {
                    ...
                    return selectedDates
                      .sort()
                      .map((dateString) => {
                        if (!slotsByDate[dateString] && !timeSlots[dateString])
                          return null;
                        ...
                        const existingTimes =
                          getExistingTimesForDate(dateString);
                            ...
                            <div className="flex justify-between items-center mb-3">
                              ...
                              <div className="flex items-center space-x-2">
                                <span className="text-sm text-gray-600">
                                  Total: {daySlots.length} | Available:{" "}
                                  {availableSlots.length} | Booked:{" "}
                                  {bookedSlots.length}
                                </span>
                                <button
                                  onClick={() =>
                                    handleRemoveAllSlotsForDate(daySlots)
                                  }
                                  ...
                                  .map((slot) => (
                                    ...
                                    >
                                      ...
                                      <button
                                        ...
                                        title={
                                          slot.is_booked
                                            ? "Remove this booked slot (will cancel appointment)"
                                            : "Remove this slot"
                                        }
                                      >
                                        ×
                                ...
                                <div className="flex justify-between items-center mb-2">
                                  <div className="flex items-center">
                                    ...
                                    <span className="ml-2 text-xs text-gray-500">
                                      ({existingTimes.length} existing time
                                      slots)
                                    </span>
                                  </div>
                                  ...
                                  {timeSlots[dateString] ? (
                                    <button
                                      onClick={() =>
                                        handleRemoveTimeSlots(dateString)
                                      }
                                    ...
                                    <button
                                      onClick={() =>
                                        handleTimeSlotSetupWithAvoidance(
                                          dateString
                                        )
                                      }
                                ...
                                {timeSlots[dateString] && (
                                  <div className="mt-3">
                                    ...
                                    <div className="mt-2 text-xs text-gray-500">
                                      {
                                        timeSlots[dateString].filter(
                                          (slot) => slot.type === "appointment"
                                        ).length
                                      }{" "}
                                ...
                                {!timeSlots[dateString] && (
                                  <div className="mt-3">
                                    ...
                                    <div className="mb-3 p-2 bg-yellow-50 border border-yellow-200 rounded text-xs text-yellow-700">
                                      <strong>Smart Conflict Avoidance:</strong>{" "}
                                      New slots will automatically skip times
                                      that conflict with existing appointments (
                                      {existingTimes.length} existing times will
                                      be avoided).
                                    </div>
                                    ...
                                    <div className="flex items-center mb-3">
                                      ...
                                      />
                                      <label
                                        htmlFor={`includeRestPeriods-${dateString}`}
                                    ...
                                    {existingTimes.length > 0 && (
                                      <div className="mt-3 p-2 bg-gray-50 border rounded text-xs">
                                        <strong>
                                          Existing times that will be avoided:
                                        </strong>
                                ...
                                <div className="text-sm text-gray-600 space-y-1">
                                  ...
                                  {timeSlots[dateString] && (
                                    <div className="text-blue-600">
                                      <strong>New Slots to Add:</strong>{" "}
                                      {
                                        timeSlots[dateString].filter(
                                          (slot) => slot.type === "appointment"
                                        ).length
                                  ...
                                  {bookedSlots.length > 0 && (
                                    <div className="text-xs text-red-500 mt-1">
                                      * Removing booked slots will cancel user
                                      appointments
                                    </div>
                                  )}
                                </div>
              ...
              {savedAppointmentData?.result?.slots &&
                savedAppointmentData.result.slots.length > 0 && (
                  <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg">
                    ...
                    <div className="text-sm text-green-700 space-y-1">
                      <div>
                        ...
                        {savedAppointmentData.result.dates?.length || 0}
                      </div>
                      <div>
                        • Total slots in database:{" "}
                        {savedAppointmentData.result.slots.length}
                      </div>
                      <div>
                        • Available slots:{" "}
                        {
                          savedAppointmentData.result.slots.filter(
                            (slot) => !slot.is_booked
                          ).length
                        }
                      </div>
                      ...
                      {removedSlots.length > 0 && (
                        <div className="text-orange-600">
                          • Slots marked for removal: {removedSlots.length}
                        </div>
                      )}
              ...
              {removedSlots.length > 0 && (
                <div className="mt-4 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                  ...
                  <div className="text-sm text-orange-700 space-y-1">
                    <div>
                      • {removedSlots.length} slot(s) will be removed when you
                      save
                    </div>
                    <div className="text-xs text-red-600">
                      * This includes any booked slots that will cancel user
                      appointments
                    </div>
                    <button
                    ...
                    return (
                      <div key={dateString} className="border rounded-md p-4">
                        <div className="flex justify-between items-center mb-2">
                          ...
                          {timeSlots[dateString] ? (
                            <button
                              onClick={() => handleRemoveTimeSlots(dateString)}
                              className="text-red-500 text-sm hover:text-red-700"
                            >
                              Remove All Slots
                            </button>
                            ...
                            <button
                              onClick={() => handleTimeSlotSetup(dateString)}
  ...
  disabled={
    isSaving ||
    (isEditingMode
      ? removedSlots.length === 0 && Object.keys(timeSlots).length === 0
      : Object.keys(timeSlots).length === 0)
  }
  className={`px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 flex items-center ${
    isSaving ||
