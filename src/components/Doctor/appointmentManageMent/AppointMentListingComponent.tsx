...
const AppointMentListingComponent = () => {
  const [selectedRequest, setSelectedRequest] = useState<Appointment | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    const handleReceiveMessage = (data: any) => {
      if (data.type === "msgReceive") {
        ...
        if (
          chatAppointment &&
          newMessageData.appointmentId === chatAppointment.id
        ) {
    ...
    try {
      ...
      if (response && response.appointments) {
        ...
        setTotalAppointments(
          response.totalAppointments || response.appointments.length
        );
        setTotalPages(
          Math.ceil(response.totalAppointments / appointmentsPerPage)
        );
      } else {
  ...
  const filterAppointmentsByDoctor = (appointments: Appointment[]) => {
    ...
    return appointments.filter((appointment) => {
      ...
      return (
        appointmentDoctorName === reduxDoctorName ||
    ...
    try {
      ...
      if (res.success) {
        const messages = res.conversations[0].messages.map((msg) => {
          const isImage =
            msg.messageType === "file" && msg.mimeType?.startsWith("image/");
      ...
      setChatMessages([
        {
          ...
          }, I have an appointment scheduled for ${formatDate(
  ...
  const searchFilteredRequests = filteredAppointments.filter(
    (appointment) =>
  ...
  const getProfileImageUrl = (name: string) => {
    return `https://ui-avatars.com/api/?name=${encodeURIComponent(
      name
    )}&background=3B82F6&color=FFFFFF&size=64&bold=true`;
  };
          ...
          <div className="mb-6 lg:mb-8">
            ...
            {doctor && (
              <p className="text-sm text-gray-500 mt-1">
                Showing appointments for: {doctorName} (
                {searchFilteredRequests.length} of {totalAppointments} total)
              </p>
            )}
          </div>
                        ...
                        {searchFilteredRequests.map((appointment) => (
                          ...
                          >
                            ...
                            <div className="flex items-center justify-between">
                              <span
                                className={`px-2 py-1 lg:px-3 lg:py-1 rounded-full text-xs font-medium border ${getUrgencyColor(
                                  appointment.status
                                )}`}
                              >
                      ...
                      <div className="space-y-4 mb-6">
                        <div className="flex items-center space-x-3">
                          ...
                          <span className="text-gray-700">
                            ...
                            {formatTime(selectedRequest.appointmentTime)}
                          </span>
                        </div>
                        <div className="flex items-center space-x-3">
                        ...
                        {selectedRequest.patientEmail && (
                          <div className="flex items-center space-x-3">
                            ...
                            <span
                              ...
                              title={selectedRequest.patientEmail}
                            >
                              {selectedRequest.patientEmail}
                            </span>
                          </div>
                    ...
                    <div className="flex items-center">
                      ...
                      <div>
                        ...
                        <p className="text-blue-200 text-xs">
                          {chatAppointment.specialty} •{" "}
                          {formatDate(chatAppointment.appointmentDate)}
                        </p>
                      </div>
                    </div>
